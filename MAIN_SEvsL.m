...
Ns = NRFr;
B = 8;
nPaths = [5:1:10]; % number of rays
SNR_dB = 0;  SNR = 10.^(SNR_dB/10)/Ns;

% Rate allocation vetors 
C1 = zeros(1,length(nPaths));
C2 = zeros(1,length(nPaths));
C3 = zeros(1,length(nPaths));
C4 = zeros(1,length(nPaths));
C5 = zeros(1,length(nPaths));
C6 = zeros(1,length(nPaths));
for Lind=1:length(nPaths)
    Ncl = nPaths(Lind);
    %Output the progress
    disp(['Progress: L = ' num2str(Lind) ' realizations.'])
    ...
    C1(Lind) = real(temp1/nChannel);
    C2(Lind) = real(temp2/nChannel);     
    C3(Lind) = real(temp3/nChannel); 
    C4(Lind) = real(temp4/nChannel);     
    C5(Lind) = real(temp5/nChannel); 
    C6(Lind) = real(temp6/nChannel); 
end
%% figure;
hold on;
plot(nPaths, C1,'r-p','Linewidth',1.5);
hold on
plot(nPaths, C2,'g->','Linewidth',1.5);
hold on
plot(nPaths, C3,'b--<','Linewidth',1.5);
hold on
plot(nPaths, C4,'Marker','d','Linewidth',1.5,'Color',[0 0.447058826684952 0.74117648601532]);
hold on
plot(nPaths, C5,'Marker','s','Linewidth',1.5,'Color',[0.36 0.08 0.18]);
hold on
plot(nPaths, C6,'m-o','Linewidth',1.5);
hold off
...
xlabel('Number of propagation paths (L)');
